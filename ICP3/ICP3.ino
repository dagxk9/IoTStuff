...
void loop() {
   float carbon_dioxide = MQGetGasPercentage(MQRead(MQ135PIN)/Ro,GAS_CARBON_DIOXIDE);
   float carbon_monoxide = MQGetGasPercentage(MQRead(MQ135PIN)/Ro,GAS_CARBON_MONOXIDE);
   float alcohol = MQGetGasPercentage(MQRead(MQ135PIN)/Ro,GAS_ALCOHOL);
   float ammonium = MQGetGasPercentage(MQRead(MQ135PIN)/Ro,GAS_AMMONIUM);
   float toluene = MQGetGasPercentage(MQRead(MQ135PIN)/Ro,GAS_TOLUENE);
   float acetone = MQGetGasPercentage(MQRead(MQ135PIN)/Ro,GAS_ACETONE);
   ...
   delay(2000);
    ...
    if ( carbon_dioxide <50 && carbon_monoxide < 50 && alcohol < 50 && ammonium < 50 && toluene <50){
      Serial.println("AIR IS GOOD");
      }
    else if  ( carbon_dioxide < 100 && carbon_monoxide < 100 && alcohol < 100 && ammonium < 100 && toluene < 100){
      Serial.println("AIR IS MODERATE");
      }
    else if ( carbon_dioxide <150 && carbon_monoxide < 150 && alcohol < 150 && ammonium < 150 && toluene <150){
      Serial.println("AIR IS UNHEALTHY FOR SENSITIVE");
      }
    else if ( carbon_dioxide <200 && carbon_monoxide < 200 && alcohol < 200 && ammonium < 200 && toluene <200){
      Serial.println("AIR IS UNHEALTHY");
      }
    else if ( carbon_dioxide <300 && carbon_monoxide < 300 && alcohol < 300 && ammonium < 300 && toluene <300){
      Serial.println("AIR IS VERY UNHEALTHY");
      }
    else if ( carbon_dioxide <500 && carbon_monoxide < 500 && alcohol < 500 && ammonium < 500 && toluene <500){
